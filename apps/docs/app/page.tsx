...
const sourceCount = readdirSync(resolve(process.cwd(), '../../docs'), { recursive: true })
  .filter((path) => typeof path === 'string' && path.endsWith('.md') && !String(path).includes('DOGFOOD') && !String(path).startsWith('landing') && !String(path).startsWith('agent-corpus'))
  .length
    ...
    <main>
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8" aria-label="Primary navigation">
        <Link href="/" className="flex items-center gap-3 font-semibold">
          <span className="grid size-9 place-items-center rounded-xl bg-[#111714] text-[#55dc91]">↔</span>
      ...
      <section className="bridge-grid border-y px-5 py-20 lg:py-28">
        <div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[1.08fr_.92fr] lg:items-center">
          <div>
            <p className="mb-5 font-mono text-xs uppercase tracking-[.2em] text-emerald-700 dark:text-emerald-300">
              Documentation infrastructure for humans + agents
            </p>
            <h1 className="max-w-4xl text-5xl font-semibold leading-[.98] tracking-[-.055em] sm:text-7xl">
              One repository.
              <br />
              Two audiences.
              <br />
              <span className="text-emerald-600">No duplicated truth.</span>
            </h1>
            <p className="mt-7 max-w-2xl text-lg leading-8 text-neutral-600 dark:text-neutral-300">
              Doc Bridge converts documentation ownership into deterministic handoffs, MCP tools, freshness gates,
              and human links—locally, before any model or backend is called.
            </p>

            <div className="mt-8 max-w-xl overflow-hidden rounded-xl border bg-[#101713] shadow-lg">
              <div className="flex items-center justify-between border-b border-white/10 px-4 py-2 font-mono text-[11px] text-white/50">
                <span>install</span>
                <span className="text-emerald-400/80">pnpm · npm · bun</span>
              </div>
              <pre className="overflow-x-auto p-4 font-mono text-sm leading-relaxed text-emerald-200">
                <code>pnpm add -D @agentskit/doc-bridge</code>
              </pre>
            </div>
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Link
                ...
                className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#111714] px-5 py-3 font-medium text-white dark:bg-white dark:text-black"
              >
          ...
          <div
            className="overflow-hidden rounded-3xl border bg-[#101713] p-2 text-[#d8e4dc] shadow-2xl shadow-emerald-950/15"
            aria-label="Doc Bridge command output"
          >
            ...
            <pre className="overflow-x-auto p-5 text-sm leading-7">
              <code>{`$ npx @agentskit/doc-bridge index
...
backend calls: 0`}</code>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8">
        <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr]">
          <div>
            <p className="font-mono text-xs uppercase tracking-[.2em] text-emerald-700 dark:text-emerald-300">The proof</p>
            <h2 className="mt-4 text-4xl font-semibold tracking-tight">Useful before AI enters the loop.</h2>
            <p className="mt-5 leading-7 text-neutral-600 dark:text-neutral-300">
            ...
            {proof.map(([title, command, description], index) => (
              <li key={title} className="grid gap-3 py-6 sm:grid-cols-[3rem_1fr] sm:gap-5">
                ...
                <div>
                  ...
                  <code className="mt-2 block overflow-x-auto rounded-lg bg-neutral-950 p-3 text-sm text-emerald-300">{command}</code>
      ...
      <section className="border-y bg-[#111714] px-5 py-20 text-white">
        <div className="mx-auto max-w-7xl">
          <p className="font-mono text-xs uppercase tracking-[.2em] text-emerald-300">Human ↔ agent loop</p>
          ...
          <div className="mt-12 grid gap-px overflow-hidden rounded-2xl bg-white/15 md:grid-cols-4">
            {(
              [
                ...
                [Braces, 'Ownership map', 'Points to source, checks, and context.'],
                [Bot, 'Agent handoff', 'Delivers compact deterministic context.'],
                [GitBranch, 'Promotion loop', 'Turns durable learnings back into docs.'],
              ] as const
            ).map(([Icon, title, copy]) => (
              <article key={title} className="bg-[#111714] p-6">
